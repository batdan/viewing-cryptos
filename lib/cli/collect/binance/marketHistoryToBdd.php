...
class marketHistoryToBdd
{
    ...
    {
        echo $this->_colorCli->getColor(' Binance | marketHistory | Start WebSocket : ' . date('H:i:s'), 'light_green') . chr(10);
    ...
    public function marketHistoryToBdd($json)
    {
        ...
        $getMhLine = $entry->data;

        // Informations sur l'ordre retournées par le WebSocket
        $id_ex      = $getMhLine->a;
        $quantity   = $getMhLine->q;
        $rate       = $getMhLine->p;
        $total      = $quantity * $rate;

        switch( $getMhLine->m ) {
            case true   : $orderType = 'SELL';  break;
            case false  : $orderType = 'BUY';   break;
        }
