...
class orderBookToBddRest
{
    ...
    private function callScripts()
    {
        ...
        foreach ($this->_marketList as $marketName => $marketNameStd) {
            ...
            if (! in_array($marketNameStd, $listOrderBookProcessActiv)) {
    ...
    public function saveOrderBook($marketName, $networkInterface)
    {
        ...
        $req = "INSERT INTO pid_pool (plateforme, service, market, pid, date_crea) VALUES ('$exchange', 'orderBook', '$marketNameStd', $pid, NOW())";
        $sql = $this->_dbh2->query($req);
        ...
        $getOrderBook = \cryptos\api\bitfinex\getOrderBook::getOrderBook($marketName, $curlOptInterface);
        ...
        // Mise en forme du json pour qu'il respecte un standart multiplateformes
...
eof;
    ...
    {
        return $this->_prefixeOrderBookTable . substr($marketName, 3, 3) . '_' . substr($marketName, 0, 3);
    }
