...
class tradingMultiDevises2
{
    ...
    private $_feesBuy           = 0.25;     // Frais de la plateforme pour un ordre d'achat
    private $_feesSell          = 0.25;     // Frais de la plateforme pour un ordre de vente
    ...
    private $_seuilMini         = 0.5;      // Seuil de gains à atteindre avant d'acheter une devise
    private $_seuilVolume       = 300;      // Volume minimum avant de se lancer dans un market
    ...
    {
        $marketSummaries = $this->_getMarket->getMarketSummaries();
    ...
    private function strategies()
    {
        ...
        $getTicker      = $this->_getMarket->getTicker('USDT-BTC');
        $usdtLast       = $getTicker->Last;
        $usdtVolSeuil   = $this->_seuilVolume * $usdtLast;

        // Calcul du volume minimum pour les market ayant l'ETH pour monnaie de référence
        $getTicker      = $this->_getMarket->getTicker('BTC-ETH');
        $ethLast        = $getTicker->Last;
        $ethVolSeuil    = $this->_seuilVolume / $ethLast;
            ...
            foreach ($this->_marketSummaries as $keySummary => $valSummary) {
                ...
                if (strstr($keySummary, $valStrat['crypto2'].'-') && $keySummary != $valStrat['market']) {
                    ...
                    foreach ($valStrat['trades'] as $keyTrade => $valTrade) {
                        ...
                        // check des volumes
                        switch ($cryptoREF) {
                            case 'USDT' : $volSeuil = $usdtVolSeuil;           break;
                            case 'ETH'  : $volSeuil = $ethVolSeuil;            break;
                            default     : $volSeuil = $this->_seuilVolume;
                        }

                        $marketName = $cryptoREF.'-'.$cryptoTDE;

                        if ($cryptoREF == 'USDT') {
                            $volBTC = $this->_seuilVolume * $usdtLast;
                        } elseif ($cryptoREF == 'ETH') {
                            $volBTC = $this->_seuilVolume / $ethLast;
                        } else {
                            $volBTC = $this->_seuilVolume;
                        }
                        ...
                        if ($this->_marketSummaries[$marketName]['BaseVolume'] < $volSeuil) {
                            echo $marketName . ' : ' . round($this->_marketSummaries[$marketName]['BaseVolume'], 2) . ' < ' . round($volSeuil, 2) . '<br>';
                        }

                        // Market de faible volume, on achête au prix de vente et on vend au prix d'achat
                        if ($keyTrade == 1 || ($keyTrade != 1 && $this->_marketSummaries[$marketName]['BaseVolume'] < $volSeuil)) {
                        ...
                        // Market de grand volume, on tente des achats et des reventes au meilleur tarif
                        } else {
                            ...
                            /*
                            $bid = $this->_marketSummaries[$marketName]['Bid'];
                            $ask = $this->_marketSummaries[$marketName]['Ask'];
